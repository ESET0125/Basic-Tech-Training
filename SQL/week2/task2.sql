...
select u1.names, b1.title, b1.price, row_number() over (partition by u1.names order by b1.price desc) as row_num,
...
select u1.names, b1.title, b1.price, row_number() over (partition by u1.users_id order by sum(b1.price)desc) as row_num,
